...
/**
 * An SynthSession is a session between the SynthServer and any other
 * agent. The session manages a particular conversation. It is created
 * by the server and uses an SynthResponder to process requests that
 * are received.
 */

public class SynthSession extends Session implements Runnable {
 ...
 */
	...
	public SynthSession (String s, Server serve, Socket sock, Responder r) {
 ...
 * are routed to the SynthResponder.
 */
	public void run () {
		try {
 ...
 */
	...
	public SynthSession appSessionTo (String to) {
		try {
			...
			for (int i=0; i<n; i++) {
				...
				SynthSession aps = (SynthSession) ias;
