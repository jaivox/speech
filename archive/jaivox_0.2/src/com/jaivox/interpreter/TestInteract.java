...
public class TestInteract extends Thread {
	...
	static void followonTests (Interact inter) {
		...
		for (int i=0; i<N; i++) {
			...
			String result = inter.execute (question);
			System.out.println (result);
			String qspec = qspecs.get (question);
			...
			for (int j=0; j<mxx; j++) {
				...
				String fspec = qspecs.get (follow);
	...
	static String messup (String question) {
		...
		if (toss > 5) {
			...
			String words [] = question.split (" ");
			// int count = (int)(Math.random ()*(words.length/2));
			int count = 2;
